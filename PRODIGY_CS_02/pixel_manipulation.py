...
def encrypt():
    if file_path and key_entry.get():
        try:
            ...
            status_label.config(text="Image encrypted Successfully :)", font=("Times New Roman", 11), fg="green")
            img_display.config(image="")  # Clear the displayed image
            key_entry.delete(0, END)  # Clear the key entry field
            
        except ValueError:
            status_label.config(text="Invalid Key: Enter an integer",font=("Times New Roman", 11), fg="red")
        except Exception as e:
            status_label.config(text=f"Encryption Failed: {e}", font=("Times New Roman", 11), fg="red")
...
def decrypt():
    if file_path and key_entry.get():
        try:
            ...
            status_label.config(text="Image Decrypted Successfully :)", font=("Times New Roman", 11), fg="green")
        except ValueError:
            status_label.config(text="Invalid Key: Enter an integer", font=("Times New Roman", 11), fg="red")
        except Exception as e:
            status_label.config(text=f"Decryption Failed: {e}", font=("Times New Roman", 11), fg="red")
...
head_lable = Label(root,text="Welcome to ImgCrypt",font=("Times New Roman", 30 ,"bold"),bg="black",fg="white")
head_lable = Label(root, text="Welcome to ImgCrypt", font=("Times New Roman", 30, "bold"), bg="black", fg="white")
head_lable.pack()
